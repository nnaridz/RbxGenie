...
function UI.build(widget: DockWidgetPluginGui): {
	...
	local statusTxt = label(strip, "Waiting...", UDim2.new(0,90,1,0), UDim2.new(0,14,0,0),
		C.subtext, 11, Enum.Font.GothamMedium)

	-- Right: port tag
	local portTag = label(strip, ":7766", UDim2.new(0,40,1,0), UDim2.new(1,-40,0,0),
		C.subtext, 10, Enum.Font.Code, Enum.TextXAlignment.Right)
	...
	-- ── API ────────────────────────────────────────────────────────────────────
	return {
		setStatus = function(connected: boolean)
			statusDot.BackgroundColor3 = if connected then C.green    else C.subtext
			statusTxt.Text            = if connected then "Online"    else "Waiting..."
			...
			statTxt.Text = if connected
				...
				else "○  Waiting for daemon on :7766"
			statTxt.TextColor3 = if connected then C.green else C.subtext
		end,

		setActive = function(tool: string?, detail: string?)
			if tool then
		...
		addEntry = function(tool: string, ok: boolean, elapsed: number?, isSystem: boolean?)
			local icon: string
